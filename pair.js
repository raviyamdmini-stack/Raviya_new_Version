...
case 'menu': {
    ...
    try {
        ...
        const ownerName = config.OWNER_NAME || 'LAKIYA';

        // 4. Prepare Image (Buffer or URL)
        const defaultImg = 'https://files.catbox.moe/vjyeby.jpg';
        const useLogo = userCfg.logo || defaultImg;
        let imageBuffer;
        
        if (String(useLogo).startsWith('http')) {
            imageBuffer = { url: useLogo }; // Use URL directly
        } else {
            try { imageBuffer = fs.readFileSync(useLogo); } catch (e) { imageBuffer = { url: defaultImg }; }
        }

        // 5. Fake Contact for Context (Optional aesthetic)
        const shonux = {
            key: {
                remoteJid: "status@broadcast",
...
│ 🤖 *Name*: ${title}
│ 👑 *Owner*: ${ownerName}
│ 🏷️ *Version*: ${config.BOT_VERSION || '1.0.0'}
│ ⏳ *Uptime*: ${hours}h ${minutes}m ${seconds}s
╰───────────────❏

> © ${config.BOT_FOOTER || 'RAVIYA MD'}
`.trim();

        // 7. Construct Interactive Message (Native Flow)
        let msgParams = {
            viewOnce: true,
            header: {
                ...
                imageMessage: await (async () => {
                    // Quick helper to upload image for header
                    // Note: Depending on your baileys version, you might need a different upload method.
                    // If this fails, simple remove 'imageMessage' and 'hasMediaAttachment'.
                    const uploaded = await socket.prepareWAMessageMedia({ image: imageBuffer }, { upload: socket.waUploadToServer });
                    return uploaded.imageMessage;
                })()
            },
            body: {
                text: text
                ...
                buttons: [
                                    ...
                                    rows: [
                                        {
                                            ...
                                            description: "Songs, Videos, APKs",
                                            id: `${config.PREFIX}download`
                                        },
                                        {
                                            header: "🎨 Creative",
                                            title: "AI & Editing",
                                            description: "Gen AI, Stickers, Fonts",
                                            id: `${config.PREFIX}creative`
                                        },
                                        {
                                            header: "🔧 Tools",
                                            title: "System Tools",
                                            description: "Group, User & Search Tools",
                    ...
                    },
                    {
                        ...
                        buttonParamsJson: JSON.stringify({
                            ...
                            url: "https://whatsapp.com/channel/your-channel-link-here",
                            merchant_url: "https://whatsapp.com/channel/your-channel-link-here"
                        })
                    }
                ]
            }
